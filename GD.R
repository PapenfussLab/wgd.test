  ...
  for(N in 0:maxN){
    ...
    for(M in lowest:maxM){
      ...
      for(i in 1:nrow(simpledata)){
        if(sum(is.na(simpledata[i,c("A","B")]))>0) next
      ...
      # ps =  new_likelihood[,1]*new_likelihood[,4]/(new_likelihood[,3]*new_likelihood[,2] +new_likelihood[,4]*new_likelihood[,1])
      # new_likelihood = ps*new_likelihood[,1]*new_likelihood[,4] + (1-ps)*new_likelihood[,2]*new_likelihood[,3]
      #ps = new_likelihood[,1]*new_likelihood[,4]/(new_likelihood[,2]*new_likelihood[,3]+new_likelihood[,1]*new_likelihood[,4])
      # print(new_likelihood)
      #likelihood = sum(log(new_likelihood[,c(1,4)])*ps)+sum(log((1-ps)*new_likelihood[,c(2,3)]))
...
for(file in all_files){
  try({
    ...
    i=i+1
    print(i)
    data = try(read.delim(paste(parent_dir,file,sep="/")))
...
outputdf[,c("lmax","lmax_half","lmax_nogd","GD","GD_half","no_GD")]
...
outputdf$GD_AIC <- outputdf$AIC_full < outputdf$AIC_half & outputdf$AIC_full < outputdf$AIC_null
outputdf$GD_half_AIC <- outputdf$AIC_half < outputdf$AIC_full & outputdf$AIC_half < outputdf$AIC_null
outputdf$no_GD_AIC <-  outputdf$AIC_null < outputdf$AIC_half & outputdf$AIC_null < outputdf$AIC_full
