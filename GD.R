        ...
        for (i in 1:nrow(simpledata)) {
          ...
          if (half) {
            if (min(simpledata[i, c("A", "B")]) == 0) {
              mylikelihood$A = getmylikelihood(min(simpledata[i, c("A", "B")]), N, M, FA =
                                                 F)
              mylikelihood$B = getmylikelihood(max(simpledata[i, c("A", "B")]), N, M, FA =
                                                 FA)
              mylikelihood_nogd$A = getmylikelihood(min(simpledata[i, c("A", "B")]), N, "-1", FA =
                                                      F)
              mylikelihood_nogd$B = getmylikelihood(max(simpledata[i, c("A", "B")]), N, "-1", FA =
                                                      FA)
            } else{
          ...
          if (is.null(mylikelihood$A) |
              ...
              is.null(mylikelihood_nogd$A) | is.null(mylikelihood_nogd$B)) {
            #print(paste("FAILED",N,M,alpha,simpledata[i,]))
            new = c(-Inf, -Inf, -Inf, -Inf)
            if (i == 1)
              new_likelihood = new
...
for (file in all_files) {
  try({
    ...
    for (chr in chr_levels) {
      ...
      chr_data_middle = chr_data[which(chr_data$start.pos < centromeres[which(centromeres$chromosome == chr), "x"] &
      ...
      meanAp = sum((chr_data_p$end - chr_data_p$start) * chr_data_p$A) / sum(chr_data_p$end -
                                                                               chr_data_p$start)
      meanBp = sum((chr_data_p$end - chr_data_p$start) * chr_data_p$B) / sum(chr_data_p$end -
                                                                               chr_data_p$start)
      startp = min(chr_data_p$start.pos, na.rm = TRUE)
      endp = max(chr_data_p$end.pos, na.rm = TRUE)
      
      meanAq = sum((chr_data_q$end - chr_data_q$start) * chr_data_q$A) / sum(chr_data_q$end -
                                                                               chr_data_q$start)
      meanBq = sum((chr_data_q$end - chr_data_q$start) * chr_data_q$B) / sum(chr_data_q$end -
                                                                               chr_data_q$start)
